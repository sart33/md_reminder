...
import solutions.mobiledev.reminder.databinding.FragmentReminderNotificationBinding
import solutions.mobiledev.reminder.domain.entity.*
import solutions.mobiledev.reminder.presentation.BaseFragment
import solutions.mobiledev.reminder.presentation.notification.ReminderNotificationFragment.Companion.UNDEFINED_ID
import solutions.mobiledev.reminder.presentation.reminder.ReminderViewModel
import solutions.mobiledev.reminder.presentation.reminder.fragment.*
import java.io.File
...
class ReminderNotificationFragment : BaseFragment<FragmentReminderNotificationBinding>() {

    private var _binding: FragmentReminderNotificationBinding? = null
    private val binding: FragmentReminderNotificationBinding
        get() = _binding ?: throw RuntimeException("FragmentReminderNotificationBinding == null")
    ...
    override fun onCreateView(
        ...
        _binding = FragmentReminderNotificationBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) = with(binding) {
            ...
            viewModel =
                ViewModelProvider(this@ReminderNotificationFragment)[ReminderViewModel::class.java]
    ...
    private fun closeNotification(notificationId: Int) {
    ...
    private fun getFormatDate(date: LocalDate) = with(binding) {
        tvReminderDate.text = dateTimeManager.getDate(date)
    ...
    private fun exitToStartScreen() {
        // создаем интент для выхода из приложения
        val exitIntent = Intent(Intent.ACTION_MAIN)
        exitIntent.addCategory(Intent.CATEGORY_HOME)
        exitIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK

        // запускаем интент и закрываем приложение
        ...
        when (requestCode) {
            REQUEST_CODE_SEND_EMAIL -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    sendEmail("lydia.yatsuhnenko@gmail.com", "Test email", "Hello, World!")
...
//            Toast.makeText(requireContext(), "Sms sent successfully", Toast.LENGTH_SHORT).show()
            ...
            imageList.firstOrNull()?.let { imageItem ->
                ...
                if (prefs.notificationBackgroundImage == 1) {
                    val file = File(
                        ...
                        ReminderItemNotificationFragment.DEFAULT_IMAGE
                    )
                    Glide.with(requireContext())
                        .load(file)
                        .into(imageView)
    ...
    private fun parseParams() {
        val args = this@ReminderNotificationFragment.arguments
        if (args != null) {
    ...
    companion object {
        const val NAME = "ReminderNotificationFragment"
        ...
        fun newInstance(remainderId: Int): ReminderNotificationFragment {
            return ReminderNotificationFragment().apply {
